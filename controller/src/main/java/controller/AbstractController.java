 ...
 *
 *         Controller controller = new ControllerBuilder(configuration, commandMap)
 ...
 * only one controller. After that you can build the middleware tree which defines in other words "roads"
 ...
 * CAN resolve which controller is executed next. But
 * <br></br>
...
public final class AbstractController extends Middleware implements Controller {
    private static final LogManager LOG_MANAGER = LogManager.createDefault(AbstractController.class);
     ...
     * Note! Query will be handled if and only if there query.name registered in the controller.
    ...
    /**
     * Do not use this method directly. It should be used only in controller module.
     */
    @Nullable
    public Response handle(String commandName,
        ...
        try {
            ...
            LOG_MANAGER.errorThrowable("Cannot create command", e);
        ...
        try {
            ...
            LOG_MANAGER.info("Command: \"" + commandName + "\" has been SUCCESSFULLY executed.");
        } catch (CommandExecutionException e) {
            LOG_MANAGER.errorThrowable("Cannot execute command", e);
            throw new ControllerException(e);
        }
